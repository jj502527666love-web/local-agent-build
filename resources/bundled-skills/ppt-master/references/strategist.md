...
**When selection includes B**, you must run `node scripts/analyze_images.cjs <project_path>/images` before outputting the spec, and integrate scan results into the image resource list.
...
The Strategist should make professional judgments on the template basis generated by `scripts/project_manager.cjs`, considering user needs, content characteristics, and audience:
...
```bash
node scripts/project_manager.cjs init <project_name> --format <canvas_format>
```
