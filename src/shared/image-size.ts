...
// ---------------------------------------------------------------------------
// Multi-image consistency (组图一致性) capability
// ---------------------------------------------------------------------------

export interface MultiImageConsistencyConfig {
  /** 单次请求最大图片数 */
  maxN: number
  /** 是否支持有参考图时使用一致性 */
...
/**
 * 已知支持原生多图一致性的模型注册表（按模型 id 前缀匹配，lowercased）。
 *
 * 注：当前为空。原先列表中的模型（gpt-image-* / seedream / wanx / kling / jimeng 等）
 * 在 OpenAI 兼容协议下都只是把 n>1 当成"生成 N 张独立图片"，
 * 不保证视觉一致性 — 实测 0.5.9 版本中 gpt-image-2 出现"两张完全不一致"或
 * "只成功一张"的情况。要做真一致需要 IP-Adapter 链式生成或厂商专门接口，
 * 暂未实现，因此先把开关下线避免误导用户。
 *
 * 后续若验证某模型确实支持原生 n>1 一致性，再加回这里即可。
 */
const CONSISTENCY_MODELS: Record<string, MultiImageConsistencyConfig> = {}

/** 查询模型是否支持多图一致性，返回配置或 null */
export function getConsistencyConfig(modelId?: string): MultiImageConsistencyConfig | null {
  if (!modelId) return null
  const lower = modelId.toLowerCase()
  // 精确匹配优先
  if (CONSISTENCY_MODELS[lower]) return CONSISTENCY_MODELS[lower]
  // 前缀匹配
  for (const [key, config] of Object.entries(CONSISTENCY_MODELS)) {
...
/** 模型是否支持多图一致性 */
export function supportsConsistency(modelId?: string): boolean {
  return getConsistencyConfig(modelId) !== null
}

/** 获取模型单次最大生成张数（不支持一致性的模型返回 1） */
export function getMaxConsistencyN(modelId?: string): number {
  return getConsistencyConfig(modelId)?.maxN ?? 1
}
