...
import { prefetchBlobs, uploadReferenced, retryPendingBlobs, QuotaExceededError } from './blob'
...
async function pullAll(scope: SyncScope, conflict: ConflictMode): Promise<{ didWork: boolean }> {
  ...
  for (let guard = 0; guard < 1000; guard++) {
    ...
    emit('merge', 0, res.changes.length, '合并变更')
    const ordered = sortForApply(res.changes)
    const db = getDatabase()
...
async function pushAll(scope: SyncScope, conflict: ConflictMode): Promise<PushOutcome> {
  ...
  if (blobRefs.length) {
    ...
    await uploadReferenced(blobRefs, (d, t) => emit('upload', d, t, '上传媒体'))
  }

  emit('push', 0, changes.length, '提交变更')
  const baseSeq = getLastPullSeq()
  const resp = await api.push(baseSeq, changes)
  if (resp.quotaExceeded) throw new QuotaExceededError()
  if (resp.needPull) return { didWork: true, needPullAgain: true }

  const byKey = new Map(changes.map((c) => [`${c.entity}|${c.uid}`, c]))
  const db = getDatabase()
  ...
  if (conflictResults.length) {
    ...
    const tx = db.transaction(() => {
      for (const r of conflictResults) applyRemoteChange(r.remote!, conflict)
    })
    tx()
  }
...
export async function runSync(): Promise<{ ok: boolean; error?: string }> {
  ...
  const fkWasOn = fkRow === 1 || fkRow === '1'
  try {
    // apply 期间临时关闭外键，规避分页导致的父子乱序落库
