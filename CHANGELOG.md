...
- **对话输入框失焦后高度缩水（`components/PromptTextarea.vue`）**：`syncAutoGrow` 以直接 DOM 方式设置的内联 `height` 会被 Vue 重渲染时的 `patchStyle` 清空（`:style` 绑定中 `height==null` 即被置空）——输入时因有 `input → 重渲染 → nextTick 重算` 闭环无感，但失焦等不伴随输入的场景下任意重渲染（补全 ghost text 消失 / hover / 父组件更新）都会让输入框回落到默认小高度。高度改由响应式 `grownHeight` 驱动，重渲染时 Vue 写回正确值；任何时刻高度只跟内容走（随输入扩展、160px 封顶后滚动、删除回落、失焦保持）。

### 变更

- **文档**：`docs/` 新增《桌面端本地打包说明.md》（云打包下线后的本地打包指引：云打包变量填充逐项 + Win `npm run dist` / Mac `npm run dist:mac` + 版本迭代 + 上传云控端 `backend/public/updates/` 热更新链路 + 常见问题）；删除四份过时内部文档（`deck-huashu-method-spec.md` / `deck-template-porting-guide.md` / `deck-base-templates-preview.html` / `ewei商城集成-实施方案.md`）；`ewei-client.ts` 注释中的文档死链同步修正（纯注释，无行为变化）。
